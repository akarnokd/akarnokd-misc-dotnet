...
{
    [MemoryDiagnoser]
    public class ShakespearePlaysScrabbleAsyncEnumerableDotNet : ShakespearePlaysScrabble
    {
        [Benchmark]
        public async ValueTask AsyncEnumerableDotNet()
        {
            await Run();
        }
        ...
        internal static async ValueTask<IList<KeyValuePair<int, IList<string>>>> Run()
        {
            ...
            Func<Func<string, IAsyncEnumerable<int>>, IAsyncEnumerable<SortedDictionary<int, IList<string>>>> buildHistoOnScore = score =>
                ...
                .Filter(word => checkBlanks(word).FirstTask().AsTask())
                .Collect(
                    () => new SortedDictionary<int, IList<string>>(IntReverse),
                    (map, word) => {
                        int key = score(word).FirstTask().AsTask().Result;
                        IList<string> list;
